<template>
    <div id="mybpmn">
      ...
      <el-drawer :visible.sync="controlDrawerVisible" size="480px" title="偏好设置" append-to-body destroy-on-close>
  ...
  // 自定义渲染（隐藏了 label 标签）
  import CustomRenderer from "@/components/Process/custom-renderer";
  // 自定义元素选中时的弹出菜单（修改 默认任务 为 用户任务）
  import CustomContentPadProvider from "@/components/Process/process-designer/plugins/content-pad";
  // 自定义左侧菜单（修改 默认任务 为 用户任务）
  import CustomPaletteProvider from "@/components/Process/process-designer/plugins/palette";
  // 自定义侧边栏
  import MyProcessPanel from "@/components/Process/process-panel/ProcessPanel";
  ...
  export default {
    ...
    data() {
      return {
        ...
        additionalModel: [CustomContentPadProvider, CustomPaletteProvider,MyProcessPanel]
        },
        addis: {}
      };
    },
