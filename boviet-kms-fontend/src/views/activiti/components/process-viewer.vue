<template>
<div class="containers main-box" v-loading="loading">
...
<script>
import {
    CustomViewer as BpmnViewer
} from "@/components/customBpmn";
...
export default {
    ...
    methods: {
        // 加载流程
        async loadFlowCanvas(processData) {
          this.loading = true;
            const self = this
            try {
                await self.bpmnViewer.importXML(processData.xmlData);
                // self.fitViewport()
                if (processData.nodeData !== undefined && processData.nodeData.length > 0) {
                    self.fillColor(processData.nodeData)
                }
            } catch (err) {
