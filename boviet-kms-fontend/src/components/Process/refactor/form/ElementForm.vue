    ...
    >
      ...
      >
        ...
        <el-form-item label="名称">
          <el-input v-model="formFieldForm.name" clearable />
        </el-form-item>
      ...
      <div class="element-drawer__button">
        ...
        <el-button size="mini" type="primary" @click="saveField"
          >保 存</el-button
        >
      </div>
    </el-drawer>
...
export default {
  ...
  methods: {
    ...
    // 保存字段配置
    saveField() {
      const { id, type, name,label, defaultValue, datePattern } = this.formFieldForm;
      const Field = window.bpmnInstances.moddle.create(
        `${this.prefix}:formProperty`,
        { id, type, name }
      );
