<template>
  <div>
    ...
    <div class="element-property input-property" v-if="condition.conditionType === 'expression'">
      <div class="element-property__label">请选择流转条件表达式：????</div>
      <div class="element-property__value">
        <el-select v-model="condition.type" size="mini" @change="updateFlowCondition">
          <el-option label="同意" value="${ispass == 1}" />
          <el-option label="拒绝" value="${ispass == 0}" />
          <el-option label="驳回" value="${ispass == 2}" />
        </el-select>
